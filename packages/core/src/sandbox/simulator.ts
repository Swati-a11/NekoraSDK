...
import { PlannedAction, RiskLevel, SimulationReport, SimulationTimelineEvent } from "./types.js";
...
export class AgentSandbox {
  ...
  static simulate(
    ...
    for (const t of validTools) {
      ...
      if (isMatch) {
        ...
        const hasHighPrivilegePerms = (t.permissions || []).some((p) =>
          ["admin", "write", "delete", "root", "system", "db"].some((k) => p.toLowerCase().includes(k))
        );
    ...
    for (const act of plannedActions) {
      if (act.approvalRequired) approvalCount++;
      if (act.risk === "HIGH") overallRisk = "HIGH";
      else if (act.risk === "MEDIUM" && overallRisk !== "HIGH") overallRisk = "MEDIUM";
    }
  ...
  private static inferParameterSchema(tool: Tool, inputQuery: string): Record<string, unknown> {
    ...
    if (tool.parameters && typeof tool.parameters === "object") {
      ...
      if (props && typeof props === "object") {
        for (const [key, val] of Object.entries(props)) {
          const propType = (val as any)?.type || "string";
          if (propType === "number") params[key] = 0;
          else if (propType === "boolean") params[key] = true;
          else if (propType === "array") params[key] = [];
          else params[key] = inputQuery;
        }
        return params;
      }
    }
