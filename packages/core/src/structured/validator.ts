...
export class StructuredOutputValidator<T> {
  ...
  parse(rawText: string): StructureValidationResult<T> {
    ...
    try {
      ...
      return {
        success: false,
        rawText,
        repairPrompt: `Your previous output was not valid JSON. Error: ${(e as Error).message}. Please output ONLY a valid JSON object matching the required schema.`,
      };
    }
