...
export class NekoraExecutionEngine {
    ...
    async execute(input, options = {}) {
        ...
        try {
            ...
            if (this.config.memory && typeof this.config.memory.retrieveContextBlock === "function") {
                const memBlock = this.config.memory.retrieveContextBlock(processedInput);
                if (memBlock) {
            ...
            const isAlreadyStored = lastStored && lastStored.role === "user" && lastStored.content === processedInput;
            if (isAlreadyStored) {
            ...
            let structuredResultData = undefined;
            ...
            while (ctx.stepCount < maxIterations) {
                ...
                let modelRes;
                if (typeof this.config.model.generateStream === "function" &&
                ...
                if (modelRes.toolCalls && modelRes.toolCalls.length > 0) {
                    ...
                    const toolResults = await this.executeToolCalls(modelRes.toolCalls, toolMap, ctx, options, emitter, tracer);
            ...
            const result = {
                runId: ctx.runId,
                sessionId: ctx.sessionId,
                output: (structuredResultData !== undefined ? structuredResultData : processedOutput),
    ...
    async executeToolCalls(toolCalls, toolMap, ctx, options, emitter, tracer) {
        const promises = toolCalls.map(async (tc) => {
            ...
            try {
                ...
                const needsApproval = targetTool.requireApproval ||
                ...
                if (this.config.pluginManager) {
                    const modArgs = await this.config.pluginManager.onBeforeToolExecution(tc.name, tc.arguments);
                    if (modArgs !== undefined)
                        argsToPass = modArgs;
                }
